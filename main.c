...
static void reverse_value(GtkWidget * widget, gpointer data)
{
  GdkRegion *region;

  GtkRange *range = (GtkRange *) widget;
  GtkWidget *cpu = (GtkWidget *) data;
  GTK_CPU(cpu)->sel = 100 - gtk_range_get_value(range);
...
int main (int argc, char ** argv)
{
  ...
  gtk_window_set_title(GTK_WINDOW(window), "ApOgEE CPU widget");
  ...
  cpu2 = gtk_cpu_new();
  gtk_fixed_put(GTK_FIXED(fixed), cpu2, 200, 40);
  GTK_CPU(cpu2)->sel = 100;
  ...
  g_signal_connect(G_OBJECT(scale), "value_changed", 
      G_CALLBACK(reverse_value), (gpointer) cpu2);

  gtk_widget_show(cpu);
  gtk_widget_show(cpu2);
